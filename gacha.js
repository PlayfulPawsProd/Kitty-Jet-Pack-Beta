...
function setupGachaLayout(canvasW, canvasH) {
    ...
    try {
        // Use gachaBackButton here
        gachaBackButton = { x: 15, y: canvasH - 65, w: 100, h: 40 }; // Same position as others
        // wardrobeTitleY = canvasH * 0.07; // Not needed here
        // shelvesY = canvasH * 0.15; // Not needed here
        machineBox = { x: canvasW / 2 - (canvasW * 0.6) / 2, y: canvasH * 0.18, w: canvasW * 0.6, h: canvasH * 0.6 };
...
function displayGacha(currentTotalPlushies) {
    ...
    const dependencies = [
        ...
        { name: 'gachaBackButton', value: gachaBackButton }, // <-- CORRECTED VARIABLE NAME!
        ...
        { name: 'backButtonColor', value: backButtonColor }, { name: 'kittyColor', value: kittyColor}
    ];
    for (const dep of dependencies) {
        if (typeof dep.value === 'undefined' || dep.value === null) {
            console.error(`Gacha display dependency missing: ${dep.name}!`);
    ...
    try { // --- Normal Drawing ---
        ...
        drawPullButton(currentTotalPlushies); drawGachaBackButton(); drawWardrobeMika(); // Reusing wardrobe draw func name, okay here
...
function drawWardrobeMika() { // Renaming this might be good later, but functionally okay
    if (!width || !height || typeof drawStaticKitty !== 'function' || typeof kittyColor === 'undefined' || !heartColor || !gachaBackButton) { return; } try { let mikaSize = min(width, height) * 0.12; let bottomBuffer = 40; let mikaX = width / 2; let mikaY = height - (mikaSize / 2) - bottomBuffer; if (mikaX - mikaSize / 2 < gachaBackButton.x + gachaBackButton.w + 10) { mikaX = gachaBackButton.x + gachaBackButton.w + 10 + mikaSize / 2; } drawStaticKitty(mikaX, mikaY, mikaSize); if (wardrobeMikaCommentaryTimer <= 0 && wardrobeMikaCommentary === "") { setWardrobeMikaCommentary("Push the button, Master! Let's see what happens!"); } if (wardrobeMikaCommentaryTimer > 0) { wardrobeMikaCommentaryTimer--; let bubbleW = width * 0.6; let bubbleH = height * 0.1; let bubbleX = mikaX - bubbleW / 2; let bubbleY = mikaY - bubbleH - mikaSize * 0.8; bubbleX = constrain(bubbleX, 5, width - bubbleW - 5); bubbleY = constrain(bubbleY, 5, height - bubbleH - 5); fill(240, 240, 240, 220); stroke(50); strokeWeight(1); rect(bubbleX, bubbleY, bubbleW, bubbleH, 10); triangle(bubbleX + bubbleW / 2 - 10, bubbleY + bubbleH, bubbleX + bubbleW / 2 + 10, bubbleY + bubbleH, mikaX, mikaY - mikaSize * 0.35); fill(50); noStroke(); textSize(min(width, height) * 0.03); textAlign(LEFT, TOP); text(wardrobeMikaCommentary, bubbleX + 15, bubbleY + 10, bubbleW - 30); textAlign(CENTER, CENTER); } else { wardrobeMikaCommentary = ""; } } catch (e) { console.error("Error drawing gacha Mika:", e); } }

// --- Animation Logic ---
function startGachaAnimation() { /* ... (Same, checks pool validity) ... */ if (gachaColorPool.length === 0 || gachaColorPool[0].startsWith('error_')) { console.error("Cannot start gacha: Pool invalid!"); setWardrobeMikaCommentary("Nya? The machine seems broken..."); return; } isGachaAnimating = true; gachaAnimationTimer = 0; currentGachaStep = 'shaking'; prizeDisplay = null; spentPlushieParticles = []; sparkParticles = []; let rawPrizeId = random(gachaColorPool); if (typeof storeItems === 'undefined') { console.error("storeItems missing!"); pendingPrizeId = null; } else { let prizeItem = storeItems.find(item => item && item.id === rawPrizeId); if (!prizeItem) { console.error(`Pulled invalid ID: ${rawPrizeId}!`); pendingPrizeId = null; } else { pendingPrizeId = prizeItem.id; console.log(`Gacha prize determined: ${pendingPrizeId} (${prizeItem.name})`); } } setWardrobeMikaCommentary("Here we go! Don't break, stupid machine!"); console.log("Gacha animation started: shaking"); }
...
function drawPrizeDisplay() { if (currentGachaStep === 'revealing' && prizeDisplay && prizeDisplay.item) { if (!machineBox || typeof getColorValueById !== 'function' || !textColor || !sparkleColor) { console.error("Deps missing for drawPrizeDisplay!"); return; } try { let item = prizeDisplay.item; let chuteCenterX = machineBox.x + machineBox.w / 2; let chuteY = machineBox.y + machineBox.h * 0.9; let displaySize = machineBox.w * 0.2; if (item.type === 'kitty_color') { let c = getColorValueById(item.id); if (c) { fill(c); } else { fill(128); } stroke(50); strokeWeight(2); rectMode(CENTER); rect(chuteCenterX, chuteY, displaySize, displaySize, 5); rectMode(CORNER); } else if (item.type === 'error') { fill(100); stroke(255,0,0); strokeWeight(2); rectMode(CENTER); rect(chuteCenterX, chuteY, displaySize, displaySize, 5); fill(255,0,0); noStroke(); textSize(displaySize*0.6); text("X", chuteCenterX, chuteY); rectMode(CORNER); } else { fill(100); stroke(50); strokeWeight(2); rectMode(CENTER); rect(chuteCenterX, chuteY, displaySize, displaySize, 5); fill(200); noStroke(); textSize(displaySize*0.6); text("?", chuteCenterX, chuteY); rectMode(CORNER); } fill(textColor); noStroke(); textSize(min(width, height) * 0.03); textAlign(CENTER, CENTER); text(item.name || "Error", chuteCenterX, chuteY + displaySize * 0.7 + 10); if (prizeDisplay.isNew && item.type !== 'error') { fill(sparkleColor); textSize(min(width, height) * 0.035); text("NEW!", chuteCenterX, chuteY - displaySize * 0.7 - 5); } } catch(e) { console.error("Error drawing prize display:", e); } } }

// --- Handle Gacha Input ---
function handleGachaInput(px, py, currentTotalPlushies) { if (isGachaAnimating) { return true; } if (!gachaBackButton || !gachaPullButton) { console.error("Gacha buttons missing!"); return false; } if (px >= gachaBackButton.x && px <= gachaBackButton.x + gachaBackButton.w && py >= gachaBackButton.y && py <= gachaBackButton.y + gachaBackButton.h) { console.log("Gacha Back!"); gameState = 'start'; wardrobeMikaCommentary = ""; wardrobeMikaCommentaryTimer = 0; return 'back'; } if (px >= gachaPullButton.x && px <= gachaPullButton.x + gachaPullButton.w && py >= gachaPullButton.y && py <= gachaPullButton.y + gachaPullButton.h) { console.log("Gacha Pull!"); if (gachaColorPool.length > 0 && !gachaColorPool[0].startsWith('error_')) { startGachaAnimation(); return 'start_pull'; } else { setWardrobeMikaCommentary("Machine looks broken... Nya..."); return 'pull_fail_pool'; } } return false; }

// --- Dependencies ---
// isItemCollected, storeItems, getColorValueById, drawStaticKitty, gameState, kittyColor, backButtonColor etc.