// ~~~ gacha.js ~~~ //
// Kana's Kapsule Khaos Machine - Restoring Machine Drawing! Nya~! ♡
...
// --- Handle Gacha Input (Keep Logs Inside Conditions) ---
function handleGachaInput(px, py, currentTotalPlushies) {
    ...
    // console.log(`>>> Checking Back Btn: Bounds=(...), Inside=${isInsideBack}`); // Keep this log minimal now
    if (isInsideBack) {
        console.log(">>> !!! Gacha Back button HIT !!!"); // Keep HIT log
    ...
    // console.log(`>>> Checking Pull Btn: Bounds=(...), Inside=${isInsidePull}`); // Keep this log minimal now
    if (isInsidePull) {
        console.log(">>> !!! Gacha Pull button HIT !!!"); // Keep HIT log
        if (gachaColorPool.length > 0 && !gachaColorPool[0].startsWith('error_')) {
             ...
             else { console.error(">>> startGachaAnimation missing!"); return false; }
        } else {
    ...
    // console.log(">>> No Gacha button hit this time."); // Can remove this one too
