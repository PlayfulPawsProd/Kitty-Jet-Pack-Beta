...
// Code for my Master~♥ Nyaa~! (Pause Button & Countdown!)
// PART 1 of 2 - Added Pause/Resume! Nya~!
...
let lives = 3;
let gameState = 'intro'; // intro, start, playing, paused, countdown, store, gameOverCutscene, gameOver
...
// --- Style Settings ---
let defaultKittyColor, jetpackColor, plushieColors = [];
let shadowKittyColor, gingerKittyColor;
...
let pauseButtonColor; // NEW: Pause Button Color
let textStrokeColor; let hudTextColorLight, hudTextColorDark;
let pauseOverlayColor; // NEW: Pause Overlay
...
let pauseButton; // NEW: Pause Button variable
...
let pauseTextSize; // Defined in setup based on canvas size
...
function setup() {
  try {
    ...
    // --- Define Colors ---
    defaultKittyColor = color(255, 105, 180); shadowKittyColor = color(80, 80, 90); gingerKittyColor = color(230, 140, 40); jetpackColor = color(150, 150, 150); plushieColors = [ color(173, 216, 230), color(255, 223, 186), color(144, 238, 144), color(221, 160, 221) ]; heartColor = color(255, 0, 0); textColor = color(250, 250, 250); textBgColor = color(0, 0, 0, 165); boomColor = color(255, 50, 50); sparkleColor = color(255, 255, 0);
    ...
    pauseButtonColor = color(200, 200, 100, 180); // Yellowish pause button
    textStrokeColor = color(0, 0, 0, 150); hudTextColorLight = color(250, 250, 250); hudTextColorDark = color(40, 40, 40);
    pauseOverlayColor = color(0, 0, 0, 150); // Dark overlay for pause
    ...
    // Define pause text size based on canvas
    pauseTextSize = min(width, height) * 0.1;
...
// --- Define Button Bounds --- (Added Pause Button)
function defineButtonBounds() {
    if (width && height) {
        let endlessButtonW = width * 0.45; let endlessButtonH = height * 0.07; endlessModeButton = { x: width / 2 - endlessButtonW / 2, y: height * 0.50, w: endlessButtonW, h: endlessButtonH };
        let storeButtonW = width * 0.35; let storeButtonH = height * 0.06; storeButton = { x: width / 2 - storeButtonW / 2, y: endlessModeButton.y + endlessButtonH * 1.8, w: storeButtonW, h: storeButtonH };
        ...
        let pauseButtonSize = min(width, height) * 0.08; // Pause button size
        pauseButton = { x: 15, y: 15, w: pauseButtonSize * 0.6, h: pauseButtonSize }; // Top Left

        if (typeof setupStoreLayout === 'function') { setupStoreLayout(width, height); } else { console.error("setupStoreLayout() function not found!"); }
    } else { console.warn("defineButtonBounds called too early."); endlessModeButton = undefined; storeButton = undefined; backButton = undefined; updateButton = undefined; pauseButton = undefined; }
}
...
function draw() { try {
    ...
    else if (score === 0 && gameState !== 'playing') { lastDifficultyIncreaseScore = -1; } // Reset if score is 0 outside of play
    ...
    // --- Draw Rainbow Trail --- (Only update if not paused/countdown)
    if (gameState === 'playing') { if (typeof updateAndDrawRainbowTrail === 'function') { updateAndDrawRainbowTrail(scrollSpeedForBackground); } else { console.warn("updateAndDrawRainbowTrail function missing!");} }
    else { if (typeof updateAndDrawRainbowTrail === 'function') { updateAndDrawRainbowTrail(0); } } // Draw existing trail without moving if paused

    // --- Screen Shake ---
    push(); if (shakeTime > 0) { translate(random(-6, 6), random(-6, 6)); shakeTime--; }

    // --- Game State Display & Music ---
    if (gameState === 'intro') { displayIntro(); manageMusic(); }
    else if (gameState === 'start') { displayStartScreen(); manageMusic(); }
    else if (gameState === 'playing') { runGame(); manageMusic(); }
    else if (gameState === 'store') { if (typeof displayStore === 'function') { displayStore(totalPlushiesCollected); } else { console.error("displayStore not found!"); gameState='start';} manageMusic(); }
    else if (gameState === 'paused') { displayPaused(); /* Music managed by state change */ } // NEW
    else if (gameState === 'countdown') { displayCountdown(); /* Music managed by state change */ } // NEW
    else if (gameState === 'gameOverCutscene') { displayGameOverCutscene(); manageMusic(); }
    else if (gameState === 'gameOver') { displayGameOverScreen(); manageMusic(); }

    lastGameState = gameState;

    // --- Draw Kitty ---
    if (!(gameState === 'intro' && introStep < 4) && gameState !== 'store') { if(kitty){ if (gameState !== 'gameOverCutscene' && gameState !== 'paused' && gameState !== 'countdown') { kitty.bobOffset = sin(frameCount * 0.1) * (kitty.size * 0.05); kitty.y = kitty.baseY + kitty.bobOffset; } else { kitty.y = kitty.baseY; } // Keep still if paused/countdown/cutscene
        drawKitty(gameState === 'gameOverCutscene');
        // Spawn particles only when playing and effect is on
...
// END OF PART 1
// PART 2 of 2 - drawKitty Bracket Fix! Nya!
...
// drawKitty - FIXED Closing Bracket Position!
function drawKitty(inCutscene = false) {
    if(!kitty) return;
    let drawBow = false;
    let currentFillColor = defaultKittyColor;
    ...
    if (typeof getEquippedItem === 'function') {
        ...
        if (equippedColorId === 'kitty_color_black') { currentFillColor = shadowKittyColor; }
        else if (equippedColorId === 'kitty_color_orange') { currentFillColor = gingerKittyColor; }
    ...
    // --- MOVED BRACKET TO THE END ---
    rectMode(CORNER); noStroke();
} // <<<--- Correct closing bracket for drawKitty()
