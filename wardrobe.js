// ~~~ wardrobe.js ~~~ //
// Mika's Super Cute Wardrobe! Bigger Mika, Sassier Commentary! Nya~! ♡
...
function drawShelfItems() { /* ... (No changes needed here, text size already adjusted) ... */ if (!itemDisplayArea || typeof isItemPurchased !== 'function' || typeof getEquippedItem !== 'function' || !storeItems || !textColor || !heartColor) { console.error("Dependencies missing for drawShelfItems!"); textSize(min(width, height)*0.04); fill(255,0,0); textAlign(CENTER, CENTER); text("Item Drawing Error!", itemDisplayArea.x + itemDisplayArea.w/2, itemDisplayArea.y + itemDisplayArea.h/2); return; } try { let itemsToShow = []; let scrollIndex = 0; let itemType = ''; if (currentShelf === 'colors') { if (typeof isItemPurchased !== 'function' || typeof storeItems === 'undefined') { throw new Error("Store functions/data not available!"); } ownedColorItems = storeItems.filter(item => item.type === 'kitty_color' && (isItemPurchased(item.id))); if (!ownedColorItems.some(item => item.id === 'default')) { ownedColorItems.unshift({ id: 'default', name: 'Default Pink', type: 'kitty_color', implemented: true }); } itemsToShow = ownedColorItems; scrollIndex = scrollIndexColors; itemType = 'kitty_color'; } else if (currentShelf === 'plushies') { textSize(min(width, height)*0.04); fill(150); textAlign(CENTER, CENTER); text("Plushie shelf coming soon, nya~!", itemDisplayArea.x + itemDisplayArea.w/2, itemDisplayArea.y + itemDisplayArea.h/2); return; } else if (currentShelf === 'upgrades') { textSize(min(width, height)*0.04); fill(150); textAlign(CENTER, CENTER); text("Upgrade shelf coming soon, nya~!", itemDisplayArea.x + itemDisplayArea.w/2, itemDisplayArea.y + itemDisplayArea.h/2); return; } if (!itemsToShow || itemsToShow.length === 0) { textSize(min(width, height)*0.04); fill(150); textAlign(CENTER, CENTER); text("Nothing here yet, Master! Go get me stuff!", itemDisplayArea.x + itemDisplayArea.w/2, itemDisplayArea.y + itemDisplayArea.h/2); return; } let displayStartX = itemDisplayArea.x + 20; let displayStartY = itemDisplayArea.y + 20; let itemSize = min(itemDisplayArea.w / (ITEMS_PER_PAGE + 1.5), itemDisplayArea.h * 0.5); let itemSpacing = (itemDisplayArea.w - 40) / ITEMS_PER_PAGE; let nameYOffset = 20; let equippedItemId = null; equippedItemId = getEquippedItem(itemType); if (itemType === 'kitty_color' && equippedItemId === null) { equippedItemId = 'default'; } for (let i = 0; i < ITEMS_PER_PAGE; i++) { let itemIndex = scrollIndex + i; if (itemIndex >= itemsToShow.length) break; let item = itemsToShow[itemIndex]; if (!item || !item.id || !item.name) continue; let itemSlotCenterX = displayStartX + i * itemSpacing + (itemSpacing / 2); let itemDrawX = itemSlotCenterX - (itemSize / 2); let itemDrawY = displayStartY + itemDisplayArea.h * 0.15; if (item.type === 'kitty_color') { if (typeof color !== 'function') throw new Error("p5 color function not available!"); let c; if(item.id === 'default') c = color(255, 105, 180); else if (item.id === 'kitty_color_black') c = color(50); else if (item.id === 'kitty_color_orange') c = color(255, 165, 0); else c = color(128); fill(c); stroke(50); strokeWeight(2); rect(itemDrawX, itemDrawY, itemSize, itemSize, 5); } else { fill(100); stroke(50); strokeWeight(2); rect(itemDrawX, itemDrawY, itemSize, itemSize, 5); fill(200); noStroke(); textSize(itemSize * 0.5); textAlign(CENTER, CENTER); text("?", itemDrawX + itemSize/2, itemDrawY + itemSize/2); } fill(textColor); noStroke(); textSize(min(width, height) * 0.022); textAlign(CENTER, TOP); text(item.name, itemSlotCenterX, itemDrawY + itemSize + nameYOffset); if (equippedItemId === item.id) { fill(heartColor); textSize(itemSize * 0.3); textAlign(CENTER, CENTER); text("♥", itemSlotCenterX, itemDrawY - 10); } } textAlign(CENTER, CENTER); } catch (e) { console.error("Error during drawShelfItems:", e); fill(255,0,0); textSize(16); textAlign(CENTER,CENTER); text(`Item Draw Error! ${e.message}`, itemDisplayArea.x + itemDisplayArea.w/2, itemDisplayArea.y + itemDisplayArea.h/2 + 20); } }

// --- Helper to Draw Arrows ---
function drawWardrobeArrows() { /* ... (logic same, check variables) ... */ if (!leftArrowButton || !rightArrowButton) { console.error("Arrow buttons not defined!"); return; } try { let items = []; let index = 0; if (currentShelf === 'colors') { items = ownedColorItems; index = scrollIndexColors; } let canScrollRight = (items && (index + ITEMS_PER_PAGE < items.length)); fill(150, 150, 180, (index > 0) ? 220 : 80); noStroke(); triangle(leftArrowButton.x + leftArrowButton.w, leftArrowButton.y, leftArrowButton.x + leftArrowButton.w, leftArrowButton.y + leftArrowButton.h, leftArrowButton.x, leftArrowButton.y + leftArrowButton.h / 2); fill(150, 150, 180, canScrollRight ? 220 : 80); noStroke(); triangle(rightArrowButton.x, rightArrowButton.y, rightArrowButton.x, rightArrowButton.y + rightArrowButton.h, rightArrowButton.x + rightArrowButton.w, rightArrowButton.y + rightArrowButton.h / 2); } catch (e) { console.error("Error drawing wardrobe arrows:", e); } }
...
// --- Helper to Draw Mika and Bubble (UPDATED SIZE/POS + Default Text) ---
function drawWardrobeMika() {
    if (!width || !height || typeof drawStaticKitty !== 'function' || !kittyColor || !heartColor || !wardrobeBackButton) {
        console.error("Dependencies missing for drawWardrobeMika!"); return;
    }
     try {
        // Slightly bigger Mika!
        let mikaSize = min(width, height) * 0.12; // <<--- SIZE INCREASED!
        // Position: Bottom Center, ensuring space above button
        let mikaX = width / 2;
        let mikaY = height - (mikaSize / 2) - 20; // Buffer from bottom

        // Ensure Mika isn't overlapping the back button (horizontally)
        if (mikaX - mikaSize / 2 < wardrobeBackButton.x + wardrobeBackButton.w + 10) { // Check left edge against button right edge
             mikaX = wardrobeBackButton.x + wardrobeBackButton.w + 10 + mikaSize / 2; // Shift right if overlapping
        }

        drawStaticKitty(mikaX, mikaY, mikaSize); // Call sketch.js function

        // Set default commentary if timer is up
        if (wardrobeMikaCommentaryTimer <= 0) {
            setWardrobeMikaCommentary("Hmm, what should I wear for Master today~?");
        }
        ...
        if (wardrobeMikaCommentaryTimer > 0) {
            ...
            let bubbleY = mikaY - bubbleH - mikaSize * 0.6; // <<--- Bubble Y position adjusted higher!
...
function handleWardrobeInput(px, py) {
    ...
    try {
        ...
        if (typeof ownedColorItems === 'undefined') { console.warn("ownedColorItems not ready."); return false; } // Safety check
        ...
        for (let i = 0; i < ITEMS_PER_PAGE; i++) { let itemIndex = indexRef.index + i; if (itemIndex >= items.length) break; let item = items[itemIndex]; if (!item) continue; let itemSlotCenterX = displayStartX + i * itemSpacing + (itemSpacing / 2); let itemDrawX = itemSlotCenterX - (itemSize / 2); let itemDrawY = displayStartY + itemDisplayArea.h*0.15;
            if (px >= itemDrawX && px <= itemDrawX + itemSize && py >= itemDrawY && py <= itemDrawY + itemSize + 40) {
                console.log("Clicked item:", item.name);
                 if (typeof equipItem !== 'function' || typeof getEquippedItem !== 'function' || typeof color !== 'function' || typeof kittyColor === 'undefined') { console.error("Equip/Get functions/color/kittyColor missing!"); shakeTime = 8; setWardrobeMikaCommentary("Nya! Something's broken!"); return true; }
                if (item.type === 'kitty_color') {
                    ...
                    if ((currentEquipped === item.id) || (item.id === 'default' && currentEquipped === null)) {
                         console.log("Unequipping color -> default."); equipItem('kitty_color', null); setWardrobeMikaCommentary("Default pink is cute too... I guess. Hmph!"); kittyColor = color(255, 105, 180);
                    } else {
                        console.log("Equipping color:", item.name); equipItem('kitty_color', item.id); setWardrobeMikaCommentary(`Yes! ${item.name} looks *purrfect* on me! Right, Master~? ♡`);
                         ...
                         else kittyColor = color(255, 105, 180);
                    }
...
// Needs access to sketch.js globals: gameState, kittyColor, shakeTime, backButtonColor, textColor, textStrokeColor, heartColor etc.
// Needs access to p5 functions: color, rect, triangle, text, fill, stroke, etc.