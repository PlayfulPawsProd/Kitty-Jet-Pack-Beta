// ~~~ wardrobe.js ~~~ //
// Mika's Super Cute Wardrobe! DEBUGGING - Mika Drawing Disabled! Nya~! ♡
...
// --- Display Wardrobe Screen (Mika Temporarily Removed) ---
function displayWardrobe() {
    ...
    try {
        ...
        drawShelfItems(); // Drawing simplified version
        drawWardrobeArrows();
        drawWardrobeBackButton();
        // drawWardrobeMika(); // <-- MIKA DRAWING TEMPORARILY COMMENTED OUT!
...
// --- Helper to Draw Shelf Items (TEMPORARILY SIMPLIFIED FOR DEBUGGING!) ---
function drawShelfItems() {
    // ** Essential Safety Check **
    if (!itemDisplayArea) { console.error("Item Display Area missing!"); return;}

    /* // --- TEMPORARILY COMMENTED OUT START ---
    // Safety checks for dependencies
    if (typeof isItemOwned !== 'function' || typeof getEquippedItem !== 'function' || typeof getColorValueById !== 'function' || !storeItems || !textColor || !heartColor) {
    ...
    try {
        ...
        if (currentShelf === 'colors') {
             if (!Array.isArray(storeItems)) throw new Error("storeItems not array!");
             let ownedColorItems = storeItems.filter(item => item && item.type === 'kitty_color' && isItemOwned(item.id));
             ...
             text("Plushie shelf soon!", itemDisplayArea.x + itemDisplayArea.w/2, itemDisplayArea.y + itemDisplayArea.h/2); return;
         } else if (currentShelf === 'upgrades') {
             textSize(min(width, height)*0.04); fill(150); textAlign(CENTER, CENTER);
             text("Upgrade shelf soon!", itemDisplayArea.x + itemDisplayArea.w/2, itemDisplayArea.y + itemDisplayArea.h/2); return;
         }

        if (!Array.isArray(itemsToShow)) { console.warn("itemsToShow invalid!"); itemsToShow = [];}
        if (itemsToShow.length === 0) {
            textSize(min(width, height)*0.04); fill(150); textAlign(CENTER, CENTER);
            text("Nothing here yet!", itemDisplayArea.x + itemDisplayArea.w/2, itemDisplayArea.y + itemDisplayArea.h/2);
            return;
        }
...
function handleWardrobeInput(px, py) { /* ... (Same as previous version - input should still work even if drawing is simplified) ... */ const neededFuncs = [isItemOwned, getEquippedItem, equipItem, setWardrobeMikaCommentary]; const neededVars = [wardrobeBackButton, shelfTabs, leftArrowButton, rightArrowButton, itemDisplayArea, storeItems, gameState, shakeTime, kittyColor, currentShelf, scrollIndexColors]; for(const func of neededFuncs) { if (typeof func !== 'function') { console.error(`Wardrobe Input missing func: ${func.name}!`); return false; }} for(const v of neededVars) { if (typeof v === 'undefined') { console.error(`Wardrobe Input missing var!`); return false; }} try { if (px >= wardrobeBackButton.x && px <= wardrobeBackButton.x + wardrobeBackButton.w && py >= wardrobeBackButton.y && py <= wardrobeBackButton.y + wardrobeBackButton.h) { console.log("Wardrobe Back!"); gameState = 'start'; return true; } for (const key in shelfTabs) { let tab = shelfTabs[key]; if (!tab) continue; if (px >= tab.x && px <= tab.x + tab.w && py >= tab.y && py <= tab.y + tab.h) { if (currentShelf !== key) { if (key === 'colors') { console.log("Switch shelf: colors"); currentShelf = key; setWardrobeMikaCommentary("Ooh, colors! Which one makes me look cutest for Master?"); } else { console.log("Shelf WIP:", key); shakeTime = 8; setWardrobeMikaCommentary("Hmph! Master hasn't unlocked this shelf for me yet!"); } } return true; } } let items = []; let indexRef = { index: 0 }; if (currentShelf === 'colors') { if(Array.isArray(storeItems)) { items = storeItems.filter(item => item && item.type === 'kitty_color' && isItemOwned(item.id)); let di=storeItems.find(i=>i&&i.id==='default'); if (di && !items.some(item => item && item.id === 'default')){ items.unshift(di); }} indexRef.index = scrollIndexColors; } if (!Array.isArray(items)) items = []; if (indexRef.index > 0 && px >= leftArrowButton.x && px <= leftArrowButton.x + leftArrowButton.w && py >= leftArrowButton.y && py <= leftArrowButton.y + leftArrowButton.h) { console.log("Scroll left:", currentShelf); indexRef.index = max(0, indexRef.index - ITEMS_PER_PAGE); if (currentShelf === 'colors') { scrollIndexColors = indexRef.index; } setWardrobeMikaCommentary("Let's see the others again!"); return true; } if ((items.length > 0 && indexRef.index + ITEMS_PER_PAGE < items.length) && px >= rightArrowButton.x && px <= rightArrowButton.x + rightArrowButton.w && py >= rightArrowButton.y && py <= rightArrowButton.y + rightArrowButton.h) { console.log("Scroll right:", currentShelf); indexRef.index += ITEMS_PER_PAGE; if (currentShelf === 'colors') { scrollIndexColors = indexRef.index; } setWardrobeMikaCommentary("What else have you got for me, Master?!"); return true; } if (items.length === 0) { return false; } let displayStartX = itemDisplayArea.x + 20; let displayStartY = itemDisplayArea.y + 20; let itemSize = min(itemDisplayArea.w / (ITEMS_PER_PAGE + 1.5), itemDisplayArea.h * 0.5); let itemSpacing = (itemDisplayArea.w - 40) / ITEMS_PER_PAGE; for (let i = 0; i < ITEMS_PER_PAGE; i++) { let itemIndex = indexRef.index + i; if (itemIndex >= items.length) break; let item = items[itemIndex]; if (!item) continue; let itemSlotCenterX = displayStartX + i * itemSpacing + (itemSpacing / 2); let itemDrawX = itemSlotCenterX - (itemSize / 2); let itemDrawY = displayStartY + itemDisplayArea.h*0.15; if (px >= itemDrawX && px <= itemDrawX + itemSize && py >= itemDrawY && py <= itemDrawY + itemSize + 40) { console.log("Clicked item:", item.name); if (item.type === 'kitty_color') { let currentEquipped = getEquippedItem('kitty_color'); if ((currentEquipped === item.id) || (item.id === 'default' && currentEquipped === null)) { console.log("Unequip color."); equipItem('kitty_color', null); setWardrobeMikaCommentary("Default pink is cute too... I guess. Hmph!"); } else { console.log("Equip color:", item.name); equipItem('kitty_color', item.id); setWardrobeMikaCommentary(`Yes! ${item.name} looks *purrfect* on me! Right, Master~? ♡`); } } else { console.log("Cannot equip:", item.type); shakeTime = 8; setWardrobeMikaCommentary("That's... not really my style, Master."); } return true; } } } catch (e) { console.error("Error during handleWardrobeInput:", e); setWardrobeMikaCommentary("Ouch! Broke something!"); return false; } return false; }
